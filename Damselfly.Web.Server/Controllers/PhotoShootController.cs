...
namespace Damselfly.Web.Server.Controllers
{
    ...
    public class PhotoShootController(PhotoShootService photoShootService, IAuthService authService)
        : Controller
    {
        ...
        public async Task<ActionResult<PhotoShootModel>> CreatePhotoShoot(
        ...
        public async Task<ActionResult<List<PhotoShootModel>>> CreatePhotoShoots(
            List<PhotoShootModel> photoShootModels
        )
        {
        ...
        [Authorize(Policy = PolicyDefinitions.s_FireBaseAdmin)]
        [ProducesResponseType(typeof(PhotoShootModel), 200)]
        public async Task<ActionResult<PhotoShootModel>> UpdatePhotoShoot(
            PhotoShootModel photoShootModel
        )
        {
        ...
        public async Task<
            ActionResult<PaginationResultModel<PhotoShootModel>>
        > GetPaginatedPhotoShoots(PhotoShootFilerRequest request)
        {
        ...
        [Route("{id}")]
        [ProducesResponseType(typeof(PhotoShootModel), 200)]
        public async Task<ActionResult<PaginationResultModel<PhotoShootModel>>> GetPhotoShootById(
            string id
        )
        {
            if (!Guid.TryParse(id, out var photoShootId))
        ...
        [ProducesResponseType(typeof(PhotoShootModel), 200)]
        public async Task<ActionResult<PhotoShootModel>> GetPhotoShootByReservationCode(
        ...
        public async Task<ActionResult<CreatePhotoShootPaymentResponse>> CreatePaymentForShoot(
            CreatePhotoShootPaymentRequest request
        )
        {
            if (request.Amount < 1)
                return BadRequest("Cannot charge less than a dollar.");
        ...
        public async Task<ActionResult<PhotoShootPaymentCaptureResponse>> CapturePaymentForShoot(
            PhotoShootPaymentCaptureRequest request
        )
        {
            if (request == null)
                return BadRequest();
            if (
                request.PaymentProcessor == Core.DbModels.Models.Enums.PaymentProcessorEnum.External
                && !await _authService.CheckCurrentFirebaseUserIsInRole(
        ...
        [ProducesResponseType(typeof(List<PhotoShootModel>), 200)]
        public async Task<ActionResult<List<PhotoShootModel>>> GetUpcomingAppointments(
        ...
        public async Task<ActionResult<BookAppointmentResponse>> ScheduleAppointment(
            ScheduleAppointmentRequest bookRequest
        )
        {
            try
            {
