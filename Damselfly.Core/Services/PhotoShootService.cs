...
namespace Damselfly.Core.Services
{
    public class PhotoShootService(
        ILogger<PhotoShootService> logger,
    ...
    )
    {
        ...
        public async Task<IEnumerable<PhotoShootModel>> CreatePhotoShoots(
            IEnumerable<PhotoShootModel> photoShoots
        )
        {
            ...
            _logger.LogInformation(
                "Creating Photo Shoots by request of {userEmail}",
                currentUserEmail
            );
        ...
        public async Task<PhotoShootModel> CreatePhotoShoot(PhotoShootModel photoShoot)
        {
            ...
            _logger.LogInformation(
                "Creating Photo Shoot for {responsibleParty} scheduled for {dateOfPhotoShoot} with {photoShootId} by request of {userEmail}",
        ...
        public async Task<PhotoShootModel?> SchedulePhotoShoot(
            ScheduleAppointmentRequest scheduleAppointmentRequest
        )
        {
            _logger.LogInformation(
                "Scheduled Photo Shoot {photoShootId} by request of {userEmail}",
                scheduleAppointmentRequest.PhotoShootId,
                scheduleAppointmentRequest.Email
            );
            var photoShoot = await _imageContext.PhotoShoots.FindAsync(
                scheduleAppointmentRequest.PhotoShootId
            );
            if (photoShoot == null)
            {
                _logger.LogError(
                    "Unable to find Photo Shoot {photoShootId} to book",
                    scheduleAppointmentRequest.PhotoShootId
                );
            ...
            {
                _logger.LogInformation(
                    "Photo Shoot {photoShootId} is already scheduled",
                    scheduleAppointmentRequest.PhotoShootId
                );
            ...
            await _bookingHubContext.Clients.All.SendAsync(
                "PhotoShootScheduled",
                photoShoot.PhotoShootId.ToString()
            );
        ...
        public async Task<PhotoShootModel> UpdatePhotoShoot(PhotoShootModel photoShoot)
        {
            ...
            {
                _logger.LogError(
                    "Unable to get current user email to UpdatePhotoShoot {photoShootId}",
                    photoShoot.PhotoShootId
                );
                return null;
            }
            _logger.LogInformation(
                "Updating Photo Shoot {photoShootId} by request of {userEmail}",
                photoShoot.PhotoShootId,
                currentUserEmail
            );

            var dbPhotoShoot = await _imageContext
                .PhotoShoots.Include(p => p.PaymentTransactions)
                .FirstAsync(p => p.PhotoShootId == photoShoot.PhotoShootId);

            // extra logic associated with an unbooking, so we need to just short circuit
            if (
                dbPhotoShoot.Status != photoShoot.Status
            ...
            if (
                dbPhotoShoot.Status != PhotoShootStatusEnum.Delivered
                && photoShoot.Status == PhotoShootStatusEnum.Delivered
            )
            {
            ...
            if (
                string.IsNullOrWhiteSpace(dbPhotoShoot.ResponsiblePartyName)
                && !string.IsNullOrWhiteSpace(photoShoot.ResponsiblePartyName)
            ...
            if (
                dbPhotoShoot.DateTimeUtc != photoShoot.DateTimeUtc
                || dbPhotoShoot.EndDateTimeUtc != photoShoot.EndDateTimeUtc
            )
            {
        ...
        {
            _logger.LogInformation(
                "Photo Shoot {photoShootId} is being marked as unbooked",
                dbPhotoShoot.PhotoShootId
            );
            ...
            await _imageContext.SaveChangesAsync();
            await _bookingHubContext.Clients.All.SendAsync(
                "PhotoShootUnscheduled",
                dbPhotoShoot.PhotoShootId.ToString()
            );
        ...
        private async Task PhotoShootDelivered(PhotoShootModel photoShoot)
        {
            ...
            {
                _logger.LogError(
                    "Unable to find album for photo shoot {photoShootId} when sending delivery email",
                    photoShoot.PhotoShootId
                );
                return;
            }
            ...
            var emailBody = EmailContent.FormatEmail(
                "Photos Delivered",
                [
        ...
        private async Task NotifiyResponsbilePartyOfCancelation(PhotoShoot photoShoot)
        {
            ...
            var emailBody = EmailContent.FormatEmail(
                ...
                [
                    "Hello!",
                    $"Your photoshoot appointment for {GetLocalTimeStringFromUtc(photoShoot.DateTimeUtc)} has been cancelled. If you have any questions please feel to email me at {questionsEmail}",
                ]
            );
            await _emailService.SendEmailAsync(
                photoShoot.ResponsiblePartyEmailAddress,
        ...
        private async Task NotifyResponsbilePartyOfScheduling(PhotoShootModel photoShoot)
        {
            if (photoShoot.ResponsiblePartyEmailAddress != null)
            {
                ...
                await _emailService.SendEmailAsync(
                    photoShoot.ResponsiblePartyEmailAddress,
                    "Photo shoot scheduled with Honey + Thyme",
                    emailBody,
                    photoShoot.PhotoShootId.ToString(),
                    MessageObjectEnum.PhotoShoot
                );
        ...
        {
            if (photoShoot.ExternalCalendarId == null)
                return;

            _logger.LogInformation(
                "Deleting calendar event for photo shoot {photoShootId} with externalCalendarId {externalCalendarId}",
                photoShoot.PhotoShootId,
                photoShoot.ExternalCalendarId
            );

            var photographer = await GetPhotographer();
            await _googleCalendarService.DeleteCalendarEventAsync(
                photographer.Id,
                photoShoot.ExternalCalendarId,
                photographer.PreferredCalendarId
            );
        }
        ...
        public async Task<bool> DeletePhotoShoot(Guid id)
        {
            ...
            {
                _logger.LogError(
                    "Unable to get current user email to DeletePhotoShoot {photoShootId}",
                    id
                );
                return false;
            }
            _logger.LogInformation(
                "Deleting Photo Shoot {photoShootId} by request of {userEmail}",
                id,
                currentUserEmail
            );
        ...
        {
            var dbPhotoShoot = await _imageContext
                .PhotoShoots.Include(x => x.PaymentTransactions.Where(t => !t.IsCancelled))
                .FirstOrDefaultAsync(x => x.PhotoShootId == id);
            if (dbPhotoShoot == null)
                return null;
        ...
        public async Task<PaginationResultModel<PhotoShootModel>> GetPhotoShootsPaginated(
            PhotoShootFilerRequest request
        )
        {
            var currentUserEmail = await _authService.GetCurrentUserEmail();
            _logger.LogInformation(
                "Getting paginated Photo Shoots for {userEmail}",
                currentUserEmail
            );
            var query = BuildPhotoShootQuery(request);
            return await Pagination.PaginateQuery(
                query,
                request.PageIndex,
                request.PageSize,
                x => MapPhotoShoot(x, _mapper)
            );
        }

        public async Task<CreatePhotoShootPaymentResponse> MakePaymentForPhotoShoot(
        ...
        )
        {
            ...
            _logger.LogInformation(
                ...
                photoShootPayment.Amount,
                photoShootPayment.ReservationCode
            );
            var photoShoot = await _imageContext.PhotoShoots.FirstOrDefaultAsync(p =>
                p.ReservationCode == photoShootPayment.ReservationCode
            );
            if (photoShoot == null)
            {
                return new CreatePhotoShootPaymentResponse
            ...
            _logger.LogInformation(
                "Creating payment for photoshoot {photoshootId} IsSuccess {isSuccess}",
                photoShoot.PhotoShootId,
                result.IsSuccess
            );
            return result;
        ...
        public async Task<bool> ValidateNotFraud()
        {
            ...
            await _cacheService.SetAsync(
                cacheKey,
                JsonConvert.SerializeObject(fraudCheckModel),
            ...
            {
                _logger.LogWarning(
                    "IP address has attempted to make payment to many times for {ip}",
                    userIp
                );
                return false;
            }
        ...
        public async Task ResetUnpaidShoots()
        {
            ...
            var photoShoots = await _imageContext
                .PhotoShoots.Where(x => x.RequestExpirationDateTime < DateTime.UtcNow)
        ...
        public async Task<PhotoShootPaymentCaptureResponse> CapturePaymentForPhotoShoot(
            PhotoShootPaymentCaptureRequest photoShootCaptureRequest
        )
        {
            try
            {
                _logger.LogInformation(
                    "Capturing payment for photoshoot with reservationCode {reservationCode} and externalOrderId {externalOrderId}",
                    photoShootCaptureRequest.ReservationCode,
                    photoShootCaptureRequest.ExternalOrderId
                );
                var photoShoot = await _imageContext.PhotoShoots.FirstOrDefaultAsync(p =>
                    p.ReservationCode == photoShootCaptureRequest.ReservationCode
                );

                if (photoShoot == null)
                {
                    _logger.LogWarning(
                        "No photoshoot found with reservationCode {reservationCode}",
                        photoShootCaptureRequest.ReservationCode
                    );
                ...
                {
                    _logger.LogWarning(
                        "There was an unrecoverable problem capturing payment for photoshoot {phtoshootId} with externalOrderId {externalOrderId}",
                        photoShoot.PhotoShootId,
                        photoShootCaptureRequest.ExternalOrderId
                    );
                    return new PhotoShootPaymentCaptureResponse
                ...
                {
                    _logger.LogWarning(
                        "There was a problem capturing payment for photoshoot {phtoshootId} with externalOrderId {externalOrderId}, but the user should try again",
                        photoShoot.PhotoShootId,
                        photoShootCaptureRequest.ExternalOrderId
                    );
                    return new PhotoShootPaymentCaptureResponse
                ...
                _logger.LogInformation(
                    "Capturing payment for photoshoot {phtoshootId} with externalOrderId {externalOrderId} was succesful",
                    photoShoot.PhotoShootId,
                    photoShootCaptureRequest.ExternalOrderId
                );
                // record transaction
                ...
                var totalPaid = await _imageContext
                    .PaymentTransactions.Where(x => x.PhotoShootId == photoShoot.PhotoShootId)
                ...
                if (photoShoot.Deposit <= totalPaid)
                {
                    if (
                        ...
                        && photoShoot.ExternalCalendarId == null
                    )
                    {
                ...
                _logger.LogInformation(
                    "Payment for photoshoot {phtoshootId} with externalOrderId {externalOrderId} was saved to the database, sending applicable emails",
                    photoShoot.PhotoShootId,
                    photoShootCaptureRequest.ExternalOrderId
                );
                ...
                if (photoShoot.ResponsiblePartyEmailAddress != null)
                {
                    ...
                    var emailBody = EmailContent.FormatEmail(
                        "Appintment confirmation",
                        [
                            $"Congrats! Your photoshoot is booked! Any remaining balance will be due the day of your shoot.",
                        ],
                        tableElements: recieptInfo
                    );
                    if (paidInFull)
                    {
                        emailBody = EmailContent.FormatEmail(
                ...
                if (paidInFull)
                    primaryMessage =
                        $"A payment of ${capture.PaymentTotal} has been recieved for the photo shoot appointment for {photoShoot.ResponsiblePartyName} {photoShoot.NameOfShoot} and is now marked as paid in full.";
                else
                    primaryMessage =
                        $"You got a booking a payment of ${capture.PaymentTotal} has been recieved for the photo shoot appointment for {photoShoot.ResponsiblePartyName} {photoShoot.NameOfShoot}";
                var adminEmailBody = EmailContent.FormatEmail(
                    "Payment Recieved",
                    ["Hello!", primaryMessage]
                );
                await _emailService.SendEmailAsync(
                    photographer.Email,
                    $"Payment Recieved For {photoShoot.ResponsiblePartyName} {photoShoot.NameOfShoot}",
                    adminEmailBody,
                    photoShoot.PhotoShootId.ToString(),
                    MessageObjectEnum.PhotoShoot
                );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Unrecoverable error trying to process payment for photoShoot with reservationCode {reservationCode}",
                    photoShootCaptureRequest.ReservationCode
                );
                return new PhotoShootPaymentCaptureResponse
                {
        ...
        internal async Task<AppIdentityUser> GetPhotographer()
        {
            ...
            return await _imageContext.Users.FirstAsync(u =>
        ...
        internal async Task CreateCalendarEvent(PhotoShoot photoShoot, AppIdentityUser photographer)
        {
            ...
            var calendarResult = await _googleCalendarService.CreateCalendarEventAsync(
                photographer.Id,
                calendarEvent
            );
        ...
        internal async Task UpdateCalendarEvent(PhotoShoot photoShoot, AppIdentityUser photographer)
        {
            ...
            var _ = await _googleCalendarService.UpdateCalendarEventAsync(
                photographer.Id,
                photoShoot.ExternalCalendarId,
                calendarEvent,
                photographer.PreferredCalendarId
        ...
        public async Task SendReminderEmails()
        {
            ...
            var photoShoots = await _imageContext
                .PhotoShoots.Where(x =>
                    x.DateTimeUtc >= startOfDay.ToUniversalTime()
                    && x.DateTimeUtc < endOfDay.ToUniversalTime()
                )
                .Where(x => !x.ReminderSent)
                .Where(x =>
                    x.Status == PhotoShootStatusEnum.Confirmed
                    || x.Status == PhotoShootStatusEnum.Paid
                )
                .Where(x =>
                    x.PaymentTransactions.Where(t => !t.IsCancelled).Sum(x => x.Amount) < x.Price
                )
                .Include(x => x.PaymentTransactions.Where(t => !t.IsCancelled))
                .ToListAsync();
            foreach (var photoShoot in photoShoots)
            {
                if (string.IsNullOrWhiteSpace(photoShoot.ResponsiblePartyEmailAddress))
                    continue;
        ...
        private PhotoShootModel MapPhotoShoot(PhotoShoot photoShoot, IMapper mapper)
        {
            ...
            var totalPaid = photoShoot.PaymentTransactions.Sum(x => x.Amount);
            ret.PaymentRemaining = photoShoot.Price - totalPaid - (photoShoot.Discount ?? 0);
            return ret;
        }
        ...
        {
            _logger.LogInformation(
                "Getting Photo Shoot by reservation code {reservationCode}",
                reservationCode
            );
            var photoShoot = await _imageContext
                .PhotoShoots.Include(x => x.PaymentTransactions.Where(t => !t.IsCancelled))
