...
namespace Damselfly.Core.Services
{
    ...
    )
    {
        ...
        public async Task<BooleanResultModel> AddImagesToAlbum(
            AddExistingImagesToAlbumRequest request
        )
        {
            var album = await _context.Albums.FirstOrDefaultAsync(a =>
                a.AlbumId == request.AlbumId
            );
            if (album == null)
                return new BooleanResultModel { Result = false };
            var images = await _context
                .Images.Where(i =>
                    request.ImageIds.Contains(i.ImageId)
                    && !i.AlbumImages.Any(a => a.AlbumId == album.AlbumId)
                )
                .ToListAsync();
            var albumImages = images
                .Select(i => new AlbumImage { AlbumId = album.AlbumId, ImageId = i.ImageId })
                .ToList();
        ...
        public async Task<AlbumModel> UpdateAlbum(AlbumModel albumModel)
        {
            ...
            var album = await _context
                .Albums.Where(a => a.AlbumId == albumModel.AlbumId)
                .FirstOrDefaultAsync();
            if (album == null)
                return null;
        ...
        {
            var affectedImages = _context
                .Images.Include(i => i.AlbumImages)
                .Where(i => i.AlbumImages.Any(a => a.AlbumId == id));
            ...
            var deleteResult = await _fileService.DeleteImages(
                new MultiImageRequest { ImageIDs = deleteImages.Select(i => i.ImageId).ToList() }
            );
        ...
        public async Task<PaginationResultModel<AlbumModel>> GetAlbumsPaginated(
            AlbumsPaginationRequest albumsPaginationRequest
        )
        {
            var query = _context.Albums.Where(a => 1 == 1);
            if (!string.IsNullOrEmpty(albumsPaginationRequest.Search))
                query = query.Where(a =>
                    a.Name.ToLower().Contains(albumsPaginationRequest.Search.ToLower())
                );
            var orderedQuery = query.OrderByDescending(a => a.CreatedDateTime);
            return await Pagination.PaginateQuery(
                orderedQuery,
                albumsPaginationRequest.PageIndex,
                albumsPaginationRequest.PageSize,
                _mapper.Map<AlbumModel>
            );
        }
        ...
        public async Task<bool> ScanAlbum(Guid albumId)
        {
            ...
            {
                var album = await _context
                    .Albums.Include(a => a.Folder)
                    .FirstOrDefaultAsync(a => a.AlbumId == albumId);
                if (album == null)
                    return false;
                var images = Directory
                    .GetFiles(album.Folder.Path, "*.*", SearchOption.AllDirectories)
                    .Where(s =>
                        s.EndsWith(".jpg")
                        || s.EndsWith(".jpeg")
                        || s.EndsWith(".png")
                ...
                await _emailService.SendEmailAsync(
                    adminEmail,
                    $"Results of {album.Name} scan",
                    stringBuilder.ToString(),
                    albumId.ToString(),
                    MessageObjectEnum.Album
                );
                await _notificationService.SendNotification(
                    "Album scan complete",
                    $"Check your email for full scan results of {album.Name}"
                );
                return true;
            }
            catch (Exception ex)
        ...
        {
            if (album.IsPublic || album.Password == null)
                return album;
            if (album.InvalidPasswordAttempts > Album.MaxInvalidPasswordAttempts)
                return null;
            if (album.Password == password)
            {
                await _context
                    .Albums.Where(x => x.AlbumId == album.AlbumId)
                    .ExecuteUpdateAsync(a => a.SetProperty(x => x.InvalidPasswordAttempts, 0));
                return album;
            }
            var isAdmin = await _authService.CheckCurrentFirebaseUserIsInRole(
            ...
            await _context
                ...
                .ExecuteUpdateAsync(a =>
                    a.SetProperty(x => x.InvalidPasswordAttempts, album.InvalidPasswordAttempts)
                );
            return null;
        }
        ...
        {
            return _context
                .Albums.Include(a => a.AlbumImages)
        ...
        private static async Task UpdateDirectoryPermissions(DirectoryInfo directory)
        {
            ...
            if (OperatingSystem.IsWindows())
            {
                directory.SetAccessControl(
                    new DirectorySecurity(directory.FullName, AccessControlSections.All)
                );
                return;
            }
