    ...
    public class PayPalPaymentProcessor : IPaymentProcessor
    {
        ...
        )
        {
            ...
            _restClient = new RestClient(
                new RestClientOptions(BaseUrl) { Timeout = new TimeSpan(0, 3, 0) }
            );
        }
        ...
        public async Task<CreateOrderResponse> CreateOrder(CreateOrderRequest orderRequest)
        {
            try
            {
                _logger.LogInformation(
                    "Creating Paypal order for invoice {invoiceId} for {amount}",
                    orderRequest.InvoiceId,
                    orderRequest.Amount
                );
                ...
                _logger.LogInformation(
                    "Creating Paypal order with request {jsonRequest}",
                    jsonRequest
                );
                ...
                _logger.LogInformation(
                    "Paypal order created with id {orderId} invoiceId {invoiceId}",
                    orderResponse.Id,
                    orderRequest.InvoiceId
                );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Error creating Paypal order for {invoiceId}",
                    orderRequest.InvoiceId
                );
        ...
        public async Task<CaptureOrderResponse> CaptureOrder(CaptureOrderRequest captureRequest)
        {
            ...
            {
                _logger.LogInformation(
                    "Capturing Paypal order {orderId} with invoiceId {invoiceId}",
                    captureRequest.PaymentProcessorOrderId,
                    captureRequest.InvoiceId
                );
                var accessToken = await GenerateAccessToken();
                var request = new RestRequest(
                    $"/v2/checkout/orders/{captureRequest.PaymentProcessorOrderId}/capture",
                    Method.Post
                );
                request.AddHeader("Content-Type", "application/json");
                request.AddHeader("Prefer", "return=representation");
                ...
                {
                    _logger.LogError(
                        "Request to Paypal could not be completed for {orderId} for invoice {invoiceId}",
                        captureRequest.PaymentProcessorOrderId,
                        captureRequest.InvoiceId
                    );
                ...
                {
                    _logger.LogError(
                        "Paypal order {orderID} was not charged successfully for invoice {invoiceId} {response}",
                        captureRequest.PaymentProcessorOrderId,
                        captureRequest.InvoiceId,
                        response.Content
                    );
                ...
                _logger.LogInformation(
                    "Paypal request was successful for order {orderID} invoice {invoiceId} {response}",
                    captureRequest.PaymentProcessorOrderId,
                    captureRequest.InvoiceId,
                    response.Content
                );
                ...
                var total = capture.PurchaseUnits.Sum(x =>
                    x.Payments.Captures.Sum(y => decimal.Parse(y.Amount.Value))
                );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Paypal order {orderId} failed during capture",
                    captureRequest.PaymentProcessorOrderId
                );
                return new CaptureOrderResponse { WasSuccessful = false, ErrorDuringCharge = true };
            }
        }
